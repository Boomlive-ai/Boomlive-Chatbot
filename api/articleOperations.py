...
from utils import read_docs, chunk_data, filter_new_urls, fetch_article_urls, store_docs_in_pinecone_vs, add_new_urls_to_database, store_old_articles
...
class StoreOldArticles(Runnable):
    async def invoke(self, *args, **kwargs):
        """
        Fetch and store old articles asynchronously.

        Returns:
            dict: A dictionary containing the status and details of the operation.
        """
        try:
            # Call the function to store old articles
            old_articles = await store_old_articles()
            
            # If the operation is successful, return a success message
            return {
                "status": "success",
                "message": "Old articles have been successfully stored.",
                "details": old_articles,
            }
        except Exception as e:
            # Handle and log any errors that occur during the process
            return {
                "status": "error",
                "message": "Failed to store old articles.",
                "error": str(e),
            }
